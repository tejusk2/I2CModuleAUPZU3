...
module I2Controller(output SCL, inout SDA, input reset_n, output logic error, input logic master_clock, output logic done, output logic [7:0]instr);
    ...
    //Vars
    localparam int NUM_INSTRUCTIONS = 63;
    ...
    logic [8:0]instruction_counter = 0; //index for byte in byte array
    ...
    logic [7:0] byte_rom [0: NUM_INSTRUCTIONS-1];//Static read only memory to configure I2C
        initial begin
            ...
            byte_rom[4] = 8'h01;
            byte_rom[5] = 8'h80;
            //Enable MICBIAS to 2.5V
            byte_rom[6] = 8'h30;
            byte_rom[7] = 8'h19; // Register 25 (0x19)
            byte_rom[8] = 8'h80; // D7-D6=10 = 2.5V, rest reserved/0
    ...
    always_ff @(posedge master_clock)begin
        ...
        if(!reset_n)begin
            ...
            if(delay_flag)begin
                if(reset_counter >= 99999999)begin//99999999
                    reset_counter <= 0;
                    delay_flag <= 0;
                end else begin
                    ...
                    if(counter== 9'd249)begin
                        ...
                        case(state)
                            ...
                            EXECUTE_STOP:begin
                                ...
                                if(reset_flag)begin
                                    if(reset_counter >= 99999)begin//99999
                                        reset_flag <= 0;
                                    end else begin
                                        reset_counter <= reset_counter + 1;
                                    end
